...
@Component({
  selector: 'app-regulation-detail',
  imports: [RouterLink],
  templateUrl: './regulation-detail.html',
})
export class RegulationDetail {
  private readonly route = inject(ActivatedRoute);
  private readonly regulationsService = inject(RegulationsService);

  readonly id = this.route.snapshot.paramMap.get('id') ?? '';
  readonly regulation = this.regulationsService.getById(this.id);
